<cfcomponent output="false">
	...
	<cffunction name="init" access="public" returntype="homePortals" hint="Constructor">
		...
		<cfscript>
			...
			variables.isHTTPS = (structKeyExists(cgi,"HTTPS") and cgi.https eq "ON");
	...
	<cffunction name="loadConfig" access="public" hint="Loads the configuration settings for the entire HomePortals server">
		<cfscript>
			...
			variables.stConfig.mailServer = "";
			variables.stConfig.mailUsername = "";
			variables.stConfig.mailPassword = "";
	...
	<cffunction name="saveConfig" access="public" hint="Saves the configuration settings for the entire HomePortals server">
		<cfscript>
			...
			// ***** Get homeportals configuration ******
			xmlConfigDoc = xmlParse(expandPath(variables.configFilePath));		
			xmlOriginalConfigDoc = xmlParse(expandPath(variables.configFilePath));		
			
			// define name for backup file
			backupFileName = ReplaceNoCase(variables.configFilePath,".xml",".bak");
			
			// save simple value settings
			lstKeys = "initialEvent,layoutSections,defaultPage,homePortalsPath,moduleLibraryPath,SSLRoot,adminEmail,mailServer,mailUsername,mailPassword,defaultAccount";
			for(i=1;i lte ListLen(lstKeys);i=i+1) {
				thisKey = ListGetAt(lstKeys,i);

				if(StructKeyExists(xmlConfigDoc.xmlRoot,thisKey))  {
					if(variables.stConfig[thisKey] neq "") {
						xmlConfigDoc.xmlRoot[thisKey].xmlText = variables.stConfig[thisKey];
					} else {
						structDelete(xmlConfigDoc.xmlRoot,thisKey);
					}
				} else {
					if(variables.stConfig[thisKey] neq "") {
		...
		<!--- store page --->
		<cffile action="write" file="#expandPath(variables.configFilePath)#" output="#toString(xmlConfigDoc)#">

		<!--- store backup --->
		<cffile action="write" file="#expandPath(backupFileName)#" output="#toString(xmlOriginalConfigDoc)#">
	...
	<cffunction name="processModules" access="public" output="false" 
		...
		<cfscript>
			...
			var k = 1;
			var location = "";
			var aLayoutSectionTypes = listToArray(variables.stConfig.layoutSections);
			var sectionType = "";
			var aSections = 0;
	...
	<cffunction name="processModule" access="private">
		...
		<cfscript>
			...
			try {
				...
				writeoutput("Processing: "  & moduleID & "   " & bIsFirstInClass & "<br>");
	...
	<cffunction name="renderModule" access="public" hint="Initializes and renders a HomePortals module instance.">
		...
		<cfoutput>
			<cfif arguments.moduleNode.Container>
				<div class="Section" id="#moduleID#">
					<div class="SectionTitle" id="#moduleID#_Head">
						<h2>
							<div class="SectionTitleLabel" id="#moduleID#_Title">#arguments.moduleNode.Title#</div>
						</h2>
					</div>
			...
			<cfelse>
				<div id="#moduleID#">
					<div id="#moduleID#_BodyRegion" style="#arguments.moduleNode.style#">
						#getPageContent("_htmlModule", moduleID)#
					</div>
				</div>
			</cfif>
	...
	<cffunction name="renderHTMLHeadCode" access="public" returntype="string" output="false">
		...
		<cfloop list="#variables.lstModulesRender#" index="moduleID">
			<cfif structKeyExists(stPageHeadContent, moduleID)>
				<cfset tmpHTML = tmpHTML & stPageHeadContent[moduleID]>
			</cfif>
		</cfloop>
	...
	<cffunction name="getDefaultAccount" access="public" returntype="any" output="false">
