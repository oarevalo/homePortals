<cfcomponent hint="This component is used to manipulate a homeportals page">
	...
	<cffunction name="addModule" access="public" returntype="void" output="False"
		...
		<cfscript>
			...
			xmlModuleNode = oCatalog.getResourceNode("module", arguments.moduleID);		
			if(xmlModuleNode eq 0) throw("The given module was not found on the catalog");
			...
			if(Not StructKeyExists(xmlModuleNode.xmlAttributes, "name") ) xmlModuleNode.xmlAttributes.name = "";
			aNodeID = xmlSearch(xmlDoc,"//module[@name='#xmlModuleNode.xmlAttributes.name#']");
			...
			tmpNode.xmlChildren[nodeIndex].xmlAttributes["name"] = xmlModuleNode.xmlAttributes.name;
			tmpNode.xmlChildren[nodeIndex].xmlAttributes["title"] = newModuleID;
			

			if(structKeyExists(xmlModuleNode,"attributes")) {
				for(i=1; i lte ArrayLen(xmlModuleNode.attributes.xmlChildren); i=i+1) {
					thisAttr = xmlModuleNode.attributes.xmlChildren[i].xmlAttributes; 
					def = "";
			...
			// add resources
			if(structKeyExists(xmlModuleNode,"resources")) {
				for(i=1; i lte ArrayLen(xmlModuleNode.resources.xmlChildren); i=i+1) {
					thisRes = xmlModuleNode.resources.xmlChildren[i].xmlAttributes; 
			...
			// add event handlers
			if(structKeyExists(xmlModuleNode,"eventListeners")) {
				aEvs = xmlModuleNode.eventListeners.xmlChildren;
