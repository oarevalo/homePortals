			...
			for(i=1;i lte qry.recordCount;i=i+1) {
				...
				if(fileExists(expandPath(tmpHREF))) {
					...
					if(fileExists(expandPath(tmpHREF))) {
						...
						if(Not StructKeyExists(variables.mapResources, qry.resType[i] & "s")) {
							variables.mapResources[qry.resType[i] & "s"] = structNew();
						}
				
						// create resource bean
						oResourceBean = createObject("component","resourceBean").init();
						oResourceBean.setID( qry.name[i] );
						oResourceBean.setHref( tmpHREF );
						oResourceBean.setPackage( qry.name[i] );
						oResourceBean.setAccessType("general");

						// add resource bean to map
						variables.mapResources[qry.resType[i] & "s"][qry.name[i]] = oResourceBean;
					}
				}
			}		
